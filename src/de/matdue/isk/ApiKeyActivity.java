...
public class ApiKeyActivity extends Activity {
	...
	private static class KeyCheckingTask extends AsyncTask<String, Void, Account> {
		...
		private void storeKey(Account account) {
			...
			newApiKey.setKey(id);
			newApiKey.setCode(code);
			
			ArrayList<de.matdue.isk.data.Character> newCharacters = new ArrayList<de.matdue.isk.data.Character>();
			for (de.matdue.isk.eve.Character eveCharacter : account.characters) {
				de.matdue.isk.data.Character newCharacter = new de.matdue.isk.data.Character();
				newCharacter.setCharacterId(eveCharacter.characterID);
				newCharacter.setName(eveCharacter.characterName);
				newCharacter.setCorporationId(eveCharacter.corporationID);
				newCharacter.setCorporationName(eveCharacter.corporationName);
				newCharacter.setSelected(true);
